...
1. [Add one or more runtime calls](#create-a-mapping-of-actions-to-their-functions) of the [Office.actions.associate](/javascript/api/office/actions#office-office-actions-associate-member) API to map a function to each action.
...
1. In the JavaScript file, use the [Office.actions.associate](/javascript/api/office/actions#office-office-actions-associate-member) API to map each action that you specified in the JSON file to a JavaScript function. Add the following JavaScript to the file. Note the following about the code.
...
1. For the body of the function, use the [Office.addin.showAsTaskpane](/javascript/api/office/addin#office-office-addin-showAsTaskpane-member(1)) method to open the add-in's task pane. When you are done, the code should look like the following:
...
1. Add a second call of `Office.actions.associate` function to map the `HIDETASKPANE` action to a function that calls [Office.addin.hide](/javascript/api/office/addin#office-office-addin-hide-member(1)). The following is an example.
...
- If the shortcut has been customized using the [Office.actions.replaceShortcuts](/javascript/api/office/actions#office-office-actions-replaceShortcuts-member) method, the value returned will be the customized keyboard combination.
