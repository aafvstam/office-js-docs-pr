...
In Message Compose mode, [getSharedPropertiesAsync](/javascript/api/outlook/messagecompose#outlook-office-messagecompose-getSharedPropertiesAsync-member(1)) is not supported in Outlook on the web or on Windows unless the following conditions are met.
