...
In Outlook Mobile, task panes take up the entire screen and by default require the user to close them to return to the message. Consider using the [Office.context.ui.closeContainer](/javascript/api/office/ui#office-office-ui-closeContainer-member(1)) method to close the task pane when your scenario is complete.
