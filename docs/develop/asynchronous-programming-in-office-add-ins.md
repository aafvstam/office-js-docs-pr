...
The following anonymous callback function declares a single parameter named `result` that retrieves data from the [AsyncResult.value](/javascript/api/office/asyncresult#office-office-asyncresult-value-member) property when the callback returns.
...
You can also use the parameter of your callback function to access other properties of the `AsyncResult` object. Use the [AsyncResult.status](/javascript/api/office/asyncresult#office-office-asyncresult-status-member) property to determine if the call succeeded or failed. If your call fails you can use the [AsyncResult.error](/javascript/api/office/asyncresult#office-office-asyncresult-error-member) property to access an [Error](/javascript/api/office/office.error) object for error information.
...
On the other hand, if you call the `Document.getSelectedDataAsync` method, it returns the data the user selected in the document to the `AsyncResult.value` property in the callback. Or, if you call the [Bindings.getAllAsync](/javascript/api/office/bindings#office-office-bindings-getAllAsync-member(1)) method, it returns an array of all of the `Binding` objects in the document. And, if you call the [Bindings.getByIdAsync](/javascript/api/office/bindings#office-office-bindings-getByIdAsync-member(1)) method, it returns a single `Binding` object.
...
- First, the [Bindings.getByIdAsync](/javascript/api/office/bindings#office-office-bindings-getByIdAsync-member(1)) method is called to access a binding in the document named "MyBinding". The `AsyncResult` object returned to the `result` parameter of that callback provides access to the specified binding object from the `AsyncResult.value` property.
- Then, the binding object accessed from the first `result` parameter is used to call the [Binding.getDataAsync](/javascript/api/office/binding#office-office-binding-getDataAsync-member(1)) method.
...
The Office JavaScript API provides the [Office.select](/javascript/api/office#Office_select_expression__callback_) method to support the promises pattern for working with existing binding objects. The promise object returned to the `Office.select` method supports only the four methods that you can access directly from the [Binding](/javascript/api/office/office.binding) object: [getDataAsync](/javascript/api/office/binding#office-office-binding-getDataAsync-member(1)), [setDataAsync](/javascript/api/office/binding#office-office-binding-setDataAsync-member(1)), [addHandlerAsync](/javascript/api/office/binding#office-office-binding-addHandlerAsync-member(1)), and [removeHandlerAsync](/javascript/api/office/binding#office-office-binding-removeHandlerAsync-member(1)).
...
The following code example uses the `select` method to retrieve a binding with the `id` "`cities`" from the `Bindings` collection, and then calls the [addHandlerAsync](/javascript/api/office/binding#office-office-binding-addHandlerAsync-member(1)) method to add an event handler for the [dataChanged](/javascript/api/office/office.bindingdatachangedeventargs) event of the binding.
...
For example, the syntax for calling the [Document.setSelectedDataAsync](/javascript/api/office/document#office-office-document-setSelectedDataAsync-member(1)) method with optional parameters inline looks like this:
