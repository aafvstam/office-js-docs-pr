...
- `getOffsetRangeAreas`: Works just like [Range.getOffsetRange](/javascript/api/excel/range#excel-excel-range-getOffsetRange-member(1)), except that a `RangeAreas` is returned and it contains ranges that are each offset from one of the ranges in the original `RangeAreas`.
