...
To add a tag to an object, call the [TagCollection.add](/javascript/api/powerpoint/tagcollection#powerpoint-powerpoint-tagcollection-add-member(1)) method of the parent object's `tags` property. The following code adds two tags to the first slide of a presentation. About this code, note:
...
1. Create a method that tags the currently selected slide as intended for `Premium` customers. About this code, note:
    ...
    - The value returned by the `getSelectedSlideIndex` function has to be decremented because the [SlideCollection.getItemAt](/javascript/api/powerpoint/slidecollection#powerpoint-powerpoint-slidecollection-getItemAt-member(1)) method is 0-based.
...
2. The following code creates a method to get the index of the selected slide. About this code, note:
    ...
    - It uses the [Office.context.document.getSelectedDataAsync](/javascript/api/office/document#office-office-document-getSelectedDataAsync-member(1)) method of the Common JavaScript APIs.
