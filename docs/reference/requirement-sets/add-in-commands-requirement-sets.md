...
\* The [Office.context.requirements.isSetSupported](/javascript/api/office/requirementsetsupport#office-office-requirementsetsupport-isSetSupported-member(1)) method will erroneously return `false` for versions 16.9 &ndash; 16.14 (inclusive), but the requirement set *is* supported on these versions.
